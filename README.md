## hey, i'm Anna 👋

Former HR & Recruiting, now I'd rather write code than job descriptions.  
Studying Software Engineering at **42 Berlin**

> I know how to read people & teams, now I'm learning how to build interfaces that do the same.

---

[![anschmit's 42 stats](https://badge.mediaplus.ma/binary/anschmit?1337Badge=off&UM6P=off)](https://github.com/oakoudad/badge42)

---

## 🎮 Graphics & Games

| Project | Description |
|---|---|
| 🎮 [cub3D](https://github.com/0vnnv0/cub3D) | 3D raycasting engine, Wolfenstein-style |
| 👾 [so_long](https://github.com/0vnnv0/so_long) | 2D game, Pac-Man-style |

## 🧠 Systems & Unix

| Project | Description |
|---|---|
| 🐚 [Minishell](https://github.com/0vnnv0/Minishell) | A fully functional shell written in C |
| 🍝 [Philosophers](https://github.com/0vnnv0/Philosophers) | Dining philosophers problem with threads |
| 💬 [minitalk](https://github.com/0vnnv0/minitalk) | Client-server communication via UNIX signals |

## ⚙️ Algorithms & Core C

| Project | Description |
|---|---|
| 🔀 [push_swap](https://github.com/0vnnv0/push_swap) | Sorting two stacks with minimal moves |
| 📘 [libft](https://github.com/0vnnv0/libft) | C standard library functions, rebuilt from scratch |
| 🖨️ [ft_printf](https://github.com/0vnnv0/printf) | printf reimplemented in C |
| ⏭️ [get_next_line](https://github.com/0vnnv0/get_next_line) | Reads any file line by line |

## 🔷 C++ Modules (CPP00 – CPP09)

[CPP00](https://github.com/0vnnv0/CPP00) · [CPP01](https://github.com/0vnnv0/CPP01) · [CPP02](https://github.com/0vnnv0/CPP02) · [CPP03](https://github.com/0vnnv0/CPP03) · [CPP04](https://github.com/0vnnv0/CPP04) · [CPP05](https://github.com/0vnnv0/CPP05) · [CPP06](https://github.com/0vnnv0/CPP06) · [CPP07](https://github.com/0vnnv0/CPP07) · [CPP08](https://github.com/0vnnv0/CPP08) · [CPP09](https://github.com/0vnnv0/CPP09)

*OOP fundamentals, templates, exceptions, casts*

---

Interested? Feel free to reach out — always happy to connect, get feedback, or hear about opportunities.
